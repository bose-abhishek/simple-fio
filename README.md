...
6. Once the setup is complete, execute `sh 02_run_tests.sh`. To run additional tests, you may change the fio parameters in the config file and re-execute `sh 02_run_tests.sh`.
...
7. Once the test is over, execute the delete script (`sh delete_simple_fio.sh --test`) to delete the test config.
8. To delete the setup (fio server / VMs and client pods) but retain the fio data use the delete script with `setup` option. 
9. To delete the project, execute the delete script with `project` option. This will delete the namespace, fio storage along with all the stored data. 
